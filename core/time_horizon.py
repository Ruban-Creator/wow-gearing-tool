...
_phase_item_names_cache: dict[int, set[str]] | None = None


def _load_phase_item_names() -> dict[int, set[str]]:
    global _phase_item_names_cache
    if _phase_item_names_cache is not None:
        return _phase_item_names_cache
    result = {}
    for phase in (3, 4, 5):
        ...
        for entries in data.get("slots", {}).values():
            for e in entries:
                names.add(e["item"])
        result[phase] = names
    _phase_item_names_cache = result
    return result


def lasts_until_phase(item_name: str) -> dict:
    """{"lasts_until_phase": N, "final_phase": bool}. N is the highest
    phase (3/4/5) whose reference list still names this item anywhere (any
    rank, any slot) - not necessarily "Best", just still a real option.
    Phase 3 is the floor (everything in the current pool is Phase 3 or
    earlier by construction); an item absent from both the Phase 4 and
    Phase 5 lists gets N=3, meaning it's expected to be replaced next
    phase. final_phase=True means it survives all the way to Phase 5,
    the confirmed last phase of TBC."""
    by_phase = _load_phase_item_names()
    last = 3
    for phase in (4, 5):
        if item_name in by_phase.get(phase, set()):
            last = phase
