...
def get_final_stats(settings_path: str, items: list[dict]) -> list[float] | None:
    """The FULL, real, fully-buffed/talented finalStats.stats array for one
    gear config via the same ComputeStats RPC get_agility() already uses -
    added 2026-09-07 so gem_optimizer.py's cap-aware gem scoring (Spell/
    Melee Hit Rating - see gem_optimizer.py's own _CAP_RATING) can read her
    TRUE total rating for any stat, not a hand-summed approximation from
    raw item/gem data (real bug found and abandoned the same day: summing
    item.scalingOptions stats + gem stats by hand misses buffs, talents,
    and set bonuses entirely - confirmed disagreeing with wowsims.com's own
    reported total by a wide margin for a real character). Cached under a
    DELIBERATELY distinct (iterations, seed) = (-1, -1) key pair - real sim
    calls always use iterations >= 500 and get_agility() itself uses (0, 0)
    - so this can never collide with either cache namespace even though
    they all share the same gear_hash/settings_fingerprint dimensions.
    Same real degrade-to-None-on-failure behavior as get_agility() (a
    ComputeStats failure here is a real, known simserver.exe issue, not
    something that should crash whatever candidate-scoring loop asked)."""
    gear_hash = gear_config.config_hash(items)
    fp = settings_fingerprint(settings_path)
    cache_key = sim_cache.key(gear_hash, fp, -1, -1)

    cached = sim_cache.get(cache_key)
    if cached is not None:
        return cached.get("final_stats")

    settings = _load_template(settings_path)
    settings["player"]["equipment"] = {"items": items}
    raid_sim_request = _build_raid_sim_request(settings, 1, 1)
    compute_stats_request = {"raid": raid_sim_request["raid"], "encounter": raid_sim_request["encounter"]}
    ...
    try:
        ...
        if result.get("errorResult"):
            raise RuntimeError(f"computeStats error: {result['errorResult'][:2000]}")
        player = result["raidStats"]["parties"][0]["players"][0]
        final_stats = player["finalStats"]["stats"]
    except (RuntimeError, KeyError, IndexError) as e:
        print(f"valuation.get_final_stats: degraded to None ({e})", file=sys.stderr)
        return None

    sim_cache.put(cache_key, {"final_stats": final_stats})
    return final_stats
