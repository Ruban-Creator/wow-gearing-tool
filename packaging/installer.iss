; Gearing Tool installer (Inno Setup 6). Real payload trimmed 2026-08-30 after
; confirming (by grepping core/adapters/gui for actual reads) what the running
; app needs from sim/tbc-new/ at runtime: only assets/database/db.json (item
; lookups) and sim/**/*.go (core/set_bonus.py's own per-class text parser) -
; db.bin is embedded into wowsimcli.exe/simserver.exe at BUILD time (go:embed),
; never read from disk at runtime, and ui/, proto/, cmd/, docs/, tools/ are
; only touched by dev-only profile-building scripts, never by a real sweep.
; That's ~8.5MB out of the submodule's real 237MB working tree.
;
...
; Output: packaging\output\GearingTool-Setup.exe

#define AppName "Gearing Tool"
#define AppPublisher "Ruban-Creator"
#define AppURL "https://github.com/Ruban-Creator/wow-gearing-tool"
#define AppExeName "gearing-tool-gui.exe"
...
DefaultDirName={localappdata}\Programs\GearingTool
DefaultGroupName={#AppName}
DisableProgramGroupPage=yes
PrivilegesRequired=lowest
OutputDir=output
OutputBaseFilename=GearingTool-Setup
Compression=lzma2
SolidCompression=yes
WizardStyle=modern
LicenseFile=..\LICENSE
; No custom icon yet (real v1 scope - see NOTES.md) - default Inno Setup
; icon is fine until the tool has its own real branding.

[Languages]
Name: "english"; MessagesFile: "compiler:Default.isl"

[Files]
Source: "..\build\dist\gearing-tool-gui.exe"; DestDir: "{app}\build\dist"; Flags: ignoreversion
